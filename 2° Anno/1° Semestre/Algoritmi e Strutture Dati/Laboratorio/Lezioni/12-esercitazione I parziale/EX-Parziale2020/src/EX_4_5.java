...
public class EX_4_5 {
	...
	public ArrayList<String> minoriDi(String[] array, String item){
		ArrayList<String> minori = new ArrayList<String>();
		for (String x: array)
			if (x.compareTo(item)<0)  minori.add(x);
		return minori;
	} //NB riusabile anche per LinkedList 
	
	
	public static ArrayList<String> minoriDi2(String[] array, String item){
		ArrayList<String> minori = new ArrayList<String>(); 
		for(int i=0;i<array.length;i++)
			if(array[i].compareTo(item) < 0) minori.add(array[i]); 
		return minori; 
	}  //NB riusabile anche per LinkedList ?
