...
public class GenericSort {
   ...
   public static <T> void insertionSort (T [ ] x, Comparator<? super T>  comp) 
   {
	   ...
	   for (i = 1; i < x.length; i++) {
		   val = x[i];
		   for (k = i; k > 0 && (comp.compare (x[k-1], val) > 0); k--) x[k] = x[k-1];
		   x[k] = val; 
	   }
   } // method insertionSort
   ...
   public static <T extends Comparable<? super T>>  void selectionSort (T [ ] x) 
   {
      ...
      for (int i = 0; i < (x.length - 1); i++) 
      {
         ...
         for (int k = i + 1; k < x.length; k++)
            if (x[k].compareTo(x [minPos]) < 0)  minPos = k;
         if (i != minPos) swap (x, i, minPos);
      } // for i
   ...
   public static <T> void selectionSort (T [ ] x, Comparator<? super T> comp) 
   {
      ...
      for (int i = 0; i < (x.length - 1); i++) 
      {
         int minPos = i;
         for (int k = i + 1; k < x.length; k++)
            if (comp.compare (x[k], x[minPos]) < 0)  minPos = k;
        swap (x, i, minPos);
      } // for i
   } // method selectionSort
   ...
   private static <T extends Comparable<? super T>> void mergeSort (T src[ ], T dest[ ], int low, int high){
    ...
    if (length < 7) {  // sorts dest[low..high-1]
        ...
        for (int i = low+1; i < high; i++) {
            val = dest[i]; 
            for (k = i; k>low && (dest[k-1]).compareTo(val) > 0;  k--) dest[k] = dest[k-1];
            dest[k] = val;
        }
        return;
     } // if length < 7
 ...
 /**
   * Sorts the array x of len objects, from index left (inclusive) 
   * into ascending order.
   */
   private static <T extends Comparable<? super T>> void quickSort(T x[ ], int left, int len){
	int right = left+len-1; // sorts x[left..right] 
       ...
       // Use Insertion Sort for small subarrays.
        if (len < 7) {
        	...
        	for (i = left + 1; i <= right; i++){
        		val = x[i];
        		for (k = i; k > left && (x[k-1]).compareTo (val) > 0; k--) x[k] = x[k-1];
        		x[k] = val;	
        	}
        	return;
        }  // if length < 7
	...
	int i = left,	
		...
		T v = x[m];  // v is the pivot

	while (true) {
		while ((x[i]).compareTo (v) < 0) i++;
		while ((x[j]).compareTo (v) > 0) j--;
		if (i < j) swap(x, i++, j--);  
		else { 
		    if (i == j) {i++; j--;}
		    break;
	...
	quickSort(x,left,j-left+1);
	quickSort(x,i,left+len-i);
