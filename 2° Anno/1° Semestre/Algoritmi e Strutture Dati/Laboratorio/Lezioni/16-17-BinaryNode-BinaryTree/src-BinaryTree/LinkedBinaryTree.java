...
public class LinkedBinaryTree<E> implements BinaryTree<E>, Iterable<E> {
    ...
    */
	...
	//Costruttore esercizio 1 Secondo parziale 2023
	...
	//FairbTree esercizio 2 Secondo appello 2023
	public static boolean fairBTreev2(LinkedBinaryTree<Integer> tree){

        return recursiveFairBTreeV2(tree.root);
    }

    protected static boolean recursiveFairBTreeV2(Node<Integer> root){

        if(root.left != null && root.right != null){
            if(root.left.data %2 != 0 && root.right.data %2 == 0) return recursiveFairBTreeV2(root.left) && recursiveFairBTreeV2(root.right);
            else return false;
        }

        if(root.left != null) if(root.left.data %2 != 0) return recursiveFairBTreeV2(root.left); else return false;

        if(root.right != null) if(root.right.data %2 == 0) return recursiveFairBTreeV2(root.right); else return false;
...
***********************************************************************/		
	...
	public boolean equals(Object other) {
		LinkedBinaryTree<?> t2;
		if(other == null || !(other instanceof LinkedBinaryTree<?>))
			return false;
	...
	public LinkedBinaryTree (E[] array) {
		...
		if (array.length == 1) {root = new Node<E> (array[0]); size = 1;}
     ...
     *  @param length: the length of the subarray to be inserted starting from the low index
     *  @return a binary tree from the specified array.
     */
	private static <E> Node<E> buildTree(E[] array, int low, int len) {
		if (len == 0) return null;
		Node<E> leftNode = buildTree(array, low + 1, (len-1)/2);
		Node<E> rightNode = buildTree(array, low + 1 + (len-1)/2, len-1-(len-1)/2);
		return new Node<E>(leftNode, array[low], rightNode);
	}
