...
private Object[] vettore = null;  // Notice that now it is an array of Object
private int size = 0;  //dimensione effettiva
...
public MyArrayList (int initialCapacity) {
	...
	vettore = new Object[initialSize];
}
...
public boolean add(E item)
{
	...
	/* 	Incrementiamo se necessario le dimensione dell'array del doppio 
		...
		vettore = Arrays.copyOf(vettore, vettore.length * 2);
	}
	vettore[size++] = item;
	return true;
}
...
public void add(int index, E item) {
	if (index < 0 || index > size) throw new IndexOutOfBoundsException();
   	if (item == null) return;
	if (size == vettore.length) vettore = Arrays.copyOf(vettore, vettore.length * 2);
...
{
	if (index < 0 || index >= size ) throw new IndexOutOfBoundsException();
  	E item = (E)vettore[index];
    for (int i=index+1; i<size; i++) vettore[i-1]=vettore[i];
    vettore[--size] = null;
