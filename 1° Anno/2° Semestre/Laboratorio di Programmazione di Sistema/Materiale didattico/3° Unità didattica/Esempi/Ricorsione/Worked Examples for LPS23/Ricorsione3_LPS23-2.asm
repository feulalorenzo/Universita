...
* Routine f
; Input
;	offset 4 da SP (word) - parametro x
; Output
;	offset 4 da SP (word) - risultato
; Registri modificati
;	d0
; Stack frame
...
; area stack frame		|				|
;						| 				|
;	0					| pc di ritorno	| allocato da chiamata - LONG
;						| 				|
;   4                   | parametro     | allocato dal chiamante - WORD
;----------------------------------------------------------------------
; dimensione stack frame: ?
f:
    ; leggo input dallo stack
    move.w  4(sp),d0
    ...
    ; calcolo il parametro per la chiamata ricorsiva
	ext.l 	d0		; sign-extension per dividendo
	divs	#3,d0	; x / 3 in d0 word

    move.w  d0,-(sp)    ; push di x / 3 nello stack
	bsr		f
	; (sp) contiene il risultato
    move.w  (sp)+,d0

	; termina calcolo
	sub.w 	4(sp),d0

    ; memorizzo il risultato nello stack
    move.w  d0,4(sp)
 	rts
...
*     "version": 1,
*     "description": "",
*     "name": "Ricorsione3_LPS23-2",
*     "language": "M68K",
*     "createdAt": 1685960485524,
*     "updatedAt": 1685962322022,
*     "id": "xKJInxL"
* }